...
export async function* take<T>(iterable: AsyncIterable<T>, limit: number): AsyncGenerator<T> {
	let i = 1
...
export async function* repeat<T>(action: () => AsyncIterable<T>): AsyncGenerator<T> {
	while (true) {
