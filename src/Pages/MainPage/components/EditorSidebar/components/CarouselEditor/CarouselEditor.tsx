...
export const CarouselEditor = () => {
	...
	useEffect(() => {
		...
		if (!activeBlock?.id && onAddBlock) {
			...
			onAddBlock({
				...
				component: <CarouselBlock images={[]} labels={[]} />,
				componentName: CompoentNames.CarouselLink,
			})
		}
	}, [activeBlock])
	...
	const getImage = async (event: ChangeEvent<HTMLInputElement>, valueProp: any) => {
		...
		// console.log(base)
		if (activeBlock?.id) {
			const currentBlock = blocks.get(
				activeBlock?.id,
			) as unknown as ComponentBlock<CarouselBlockProps>
			...
			if (currentBlock) {
				...
				onAddBlock({
					...currentBlock,
					component: (
						<CarouselBlock images={currentBlock.value.images} labels={currentBlock.value.labels} />
					),
				})
			}
		}
		...
		<div>
			...
			{selectedImage &&
				selectedImage.map((el, i) => (
					<Input
						key={(activeBlock as ComponentBlock<CarouselBlockProps>)?.value.text}
						value={(activeBlock as ComponentBlock<CarouselBlockProps>)?.value.text}
